...
## 🔐 Production Credentials - КРИТИЧЕСКИ ВАЖНО

**ЕДИНЫЙ ФОРМАТ для всей системы (Migration 010):**
...
**⚠️ ПРАВИЛА:**
- ✅ ВСЕГДА используй ТОЛЬКО эти credentials
- ✅ ВСЕ сметы/каталоги сохраняются в `magellania-org`
- ✅ ВСЕ операции выполняются под `superadmin`
...
- 📝 См. документацию: `API_V1_LOGIN_FIX.md`

**Multi-tenancy:**
- Архитектура multi-tenancy ОСТАЁТСЯ без изменений
- Сейчас используется только одна организация (magellania-org)
- В будущем можно добавить другие организации

---

## ⚡ Для разработчиков - НАЧАТЬ ЗДЕСЬ

**🗺️ CODE MAP** - Навигация по коду для эффективной разработки

При любой задаче разработки (баг, новая функция, рефакторинг, интеграция) **СНАЧАЛА** прочитайте:

📋 **[docs/CODE_MAP.md](docs/CODE_MAP.md)** - Подробная карта кода с:
- Точными расположениями функций (строки)
- Caller/callee графами зависимостей
- Известными проблемами и race conditions
- Архитектурными слоями
- Integration points

**Зачем это нужно:**
- ✅ Bug fixing: 5-7 мин → 30-60 сек (10x быстрее)
- ✅ Новая функция: 5-10 мин → 1-2 мин (5x быстрее)
- ✅ Рефакторинг: 10-15 мин → 2-3 мин (5x быстрее)
- ✅ Интеграция: 20-30 мин → 5-7 мин (4x быстрее)

**Как использовать:**
```markdown
Задача: Исправить баг с autosave
1. Читаю CODE_MAP.md → секция "Known Issues & Race Conditions"
2. Вижу: isLoadingQuote guard flag, строки 9430, 9488, 9533
3. Открываю index.html:9430 - сразу в нужном месте
4. Фиксю за 1 минуту вместо 10 минут поиска
```
...
### Полная документация - MkDocs Wiki

**ВАЖНО:** Вся подробная документация перенесена в MkDocs структуру:

📖 **Главная страница:** `docs/index.md`

**Для пользователей:**
- 👤 [Руководство пользователя](docs/ru/user-guide/index.md) - установка, работа с программой
- 📖 [FAQ](docs/ru/user-guide/troubleshooting/faq.md) - частые вопросы

**Для разработчиков:**
- 👨‍💻 [Руководство разработчика](docs/ru/developer-guide/index.md) - архитектура, API, разработка
- 🏗️ [Архитектура](docs/ru/developer-guide/architecture/index.md) - дизайн системы
- 🔧 [API Reference](docs/ru/developer-guide/api-reference/index.md) - REST API документация

**🔥 КРИТИЧНО - Обязательно к прочтению:**
- [Целостность данных](docs/ru/developer-guide/data-integrity/index.md) - **P0-приоритет**
- [ID-First Pattern](docs/ru/developer-guide/data-integrity/id-first-pattern.md) - UUID как ключ
- [Single Source of Truth](docs/ru/developer-guide/data-integrity/single-source-truth.md) - одна таблица
- [Data Flow](docs/ru/developer-guide/data-integrity/data-flow.md) - потоки данных
...
## 🔥 КРИТИЧЕСКИ ВАЖНО - Правила защиты архитектуры

### ⚠️ ОБЯЗАТЕЛЬНО выполнять ДО любых изменений в коде!

**Перед изменением кода, связанного с данными, ВСЕГДА:**

1. **Прочитайте документацию Data Integrity** (15-30 минут):
   - [docs/ru/developer-guide/data-integrity/index.md](docs/ru/developer-guide/data-integrity/index.md)
   - [docs/ru/developer-guide/data-integrity/id-first-pattern.md](docs/ru/developer-guide/data-integrity/id-first-pattern.md)
   - [docs/ru/developer-guide/data-integrity/single-source-truth.md](docs/ru/developer-guide/data-integrity/single-source-truth.md)

2. **Проверьте Checklist:**
   - [ ] ВСЕ операции используют `id` (UUID) как ключ, НЕ `filename`
   - [ ] filename используется ТОЛЬКО для UI display
   - [ ] ТОЛЬКО `estimates` table для runtime данных
   - [ ] НЕТ dual storage (estimates + backups одновременно)
   - [ ] Optimistic locking работает (data_version проверяется)
   - [ ] Guard flags для предотвращения race conditions

3. **Золотые правила - НЕЛЬЗЯ нарушать:**

```javascript
// ✅ ПРАВИЛЬНО - ID-First
loadEstimate(id)           // UUID как ключ
saveEstimate(id, data)     // UUID в URL
deleteEstimate(id)         // UUID для операций
renameEstimate(id, newFilename)  // ID + новое имя

// ❌ НЕПРАВИЛЬНО - filename как ключ
loadEstimate(filename)     // НЕТ! filename может меняться
saveEstimate(filename, data)  // НЕТ! не уникален

// ✅ ПРАВИЛЬНО - Single Source
await storage.saveEstimate(id, data);  // Один источник

// ❌ НЕПРАВИЛЬНО - Dual Storage
await storage.saveEstimate(id, data);
await storage.saveBackup(id, data);    // НЕТ! Рассинхронизация!

// ✅ ПРАВИЛЬНО - filename только для UI
const displayName = filename.replace(/\.json$/i, '');
element.textContent = displayName;

// ❌ НЕПРАВИЛЬНО - filename в WHERE
SELECT * FROM estimates WHERE filename = ?  // НЕТ! Не ключ!
```

### 🚫 Категорически ЗАПРЕЩЕНО:

1. **НЕ использовать filename как primary key**
   - Filename может меняться (rename)
   - Filename не уникален
   - Всегда используйте UUID (id)

2. **НЕ создавать dual storage**
   - НЕ сохранять одновременно в estimates И backups
   - НЕ синхронизировать между таблицами
   - Один источник истины - `estimates` table

3. **НЕ обходить optimistic locking**
   - Всегда проверяйте `data_version` при UPDATE
   - Increment версию при успехе
   - Обрабатывайте ошибки concurrent modification

4. **НЕ игнорировать guard flags**
   - `isLoadingQuote` предотвращает autosave во время load
   - Race conditions приводят к "прилипанию" данных

### ⚡ Последствия нарушений:

- ❌ Потеря данных пользователей
- ❌ Рассинхронизация estimates/backups
- ❌ ENOENT ошибки после rename
- ❌ Services "прилипают" между сметами
- ❌ Невозможность загрузить сметы
- ❌ Конфликты конкурентных изменений

### 📋 Обязательная документация при изменениях:

**При добавлении нового функционала:**
- Обновите соответствующие разделы в `docs/`
- Используйте шаблоны из `docs/_templates/`
- Добавьте тесты, подтверждающие соблюдение паттернов
- Обновите CHANGELOG в `docs/ru/developer-guide/history/changelog.md`
