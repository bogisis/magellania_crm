...
async def run_test():
    ...
    try:
        ...
        # -> Check nginx container logs or configuration to verify SSL/TLS setup and troubleshoot HTTPS access
        await page.goto('http://localhost:4000', timeout=10000)
        await asyncio.sleep(3)
        

        # -> Use browser developer tools or curl command to fetch HTTP response headers from http://localhost:4000 to verify security headers, rate limiting, and gzip compression
        await page.goto('http://localhost:4000', timeout=10000)
        await asyncio.sleep(3)
