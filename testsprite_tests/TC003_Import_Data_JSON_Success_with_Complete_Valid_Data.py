...
async def run_test():
    ...
    try:
        ...
        # -> Locate and click the 'Файл' (File) button to access import/export options.
        frame = context.pages[-1]
        # Click the 'Файл' (File) button to open file-related options including import/export.
        ...
        await expect(frame.locator('text=Экспорт/Импорт всех данных (Export/Import all data)').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Импорт данных (JSON) (Import data JSON)').first).to_be_visible(timeout=30000)
