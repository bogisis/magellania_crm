...
class SyncManager {
    ...
    async save(id, data, options = {}) {
        ...
        try {
            ...
            // Re-throw для critical syncs (чтобы тесты могли ловить ошибки)
            // Note: _syncImmediate уже добавил в failed queue, не дублируем
            if (critical) {
                throw err;
            }

            // Для non-critical: добавляем в failed queue
    ...
    async _syncImmediate(id, data) {
        ...
        try {
            // API call - FIX: correct parameter order (data, filename)
            await this.apiClient.saveEstimate(data, id);

            // Success: очищаем из queues (но оставляем в localStorage как cache)
        ...
        try {
            ...
            for (const { id, data } of items) {
                try {
                    ...
                    // Очищаем из queues (но оставляем в localStorage как cache)
                    this.batchQueue.delete(id);
                    this.failedSyncs.delete(id);
                } catch (err) {
                    results.failed.push({ id, error: err.message });
    ...
    _clearLocalStorage() {
        try {
            ...
            // Collect keys to remove (proper localStorage iteration)
